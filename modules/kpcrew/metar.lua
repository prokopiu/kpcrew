...
local weatherlib   = require("kpcrew.weatherlib")
    ...
    if dir:match('^%d%d%d$') then
        ...
        for _, test_direction in pairs(token_data.wind.directions) do
            if (
                ...
                direction = dir --WIND_DIRECTION[test_direction.key]
                break
            end
        end
    else
        direction = dir --WIND_DIRECTION[dir]
    end
...
local function parse_metar_vrb_wind(dir, speed)
    local data = { direction = "VRB", speed = tonumber(speed) }
    return { wind = data }
end
...
local function parse_metar_vis_sm(visibility)
    local data = { distance = visibility * 1604 }
    return { visibility = data }
end

local function parse_metar_vis_fractsm(main, bruch)
    local data = { distance = tonumber(main) / tonumber(bruch) * 1604 }
    return { visibility = data }
end
...
local function parse_metar_pressure(unit, pressure)
    if unit == 'A' then
        pressure = tonumber(pressure) / 100
    else
...
local metar_token_handlers = {
    ...
    {
        pattern = '^(%d%d%d)(%d%d+)G(%d+)(KT)$',
        handler = parse_metar_wind,
    },
    {
        pattern = '^(%d%d%d)(%d%d+)(KT)$',
        handler = parse_metar_wind,
    },
    {
        pattern = '^(VRB)(%d%d+)(KT)$',
        handler = parse_metar_vrb_wind,
    },
    {
        pattern = '^(%d*)(SM)$',
        handler = parse_metar_vis_sm,
    },
    {
        pattern = '^(%d)/(%d)(SM)$',
        handler = parse_metar_vis_fractsm,
    },
    {
        pattern = '^(%d%d%d%d)([A-Z]*)$',
...
function metatable.__index:get_metar_data(metar_string)
