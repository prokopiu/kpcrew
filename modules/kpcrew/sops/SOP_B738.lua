...
electricalPowerUpProc:addItem(ProcedureItem:new("TAKEOFF CONFIG WARNING","TEST",ChecklistItem.actorFO,1,function (self) if get("laminar/B738/system/takeoff_config_warn") > 0 and takeoffwarn == nil then takeoffwarn = 1 end return takeoffwarn == 1 end,nil,nil))
...
electricalPowerUpProc:addItem(ProcedureItem:new("  OVHT FIRE TEST SWITCH","HOLD RIGHT",ChecklistItem.actorFO,1,function (self) if sysEngines.ovhtFireTestSwitch:getStatus() > 0 and firetest == nil then firetest = 1 end return firetest == 1 end,nil,nil))
electricalPowerUpProc:addItem(ProcedureItem:new("  MASTER FIRE WARN LIGHT","PUSH",ChecklistItem.actorFO,1,function (self) if sysGeneral.fireWarningAnc:getStatus() > 0 and firewarn == nil then firewarn = 1 end return firewarn == 1 end,nil,nil))
electricalPowerUpProc:addItem(ProcedureItem:new("  ENGINES EXT TEST SWITCH","TEST 1 TO LEFT",ChecklistItem.actorFO,1,function (self) if get("laminar/B738/toggle_switch/extinguisher_circuit_test") < 0 and engext1 == nil then engext1 = 1 end return engext1 == 1 end,nil,nil))
electricalPowerUpProc:addItem(ProcedureItem:new("  ENGINES EXT TEST SWITCH","TEST 2 TO RIGHT",ChecklistItem.actorFO,1,function (self) if get("laminar/B738/toggle_switch/extinguisher_circuit_test") > 0 and engext2 == nil then engext2 = 1 end return engext2 == 1 end,nil,nil))
...
prelPreflightProc:addItem(ProcedureItem:new("ATTENDENCE BUTTON","PRESS",ChecklistItem.actorFO,1,function (self) if sysGeneral.attendanceButton:getStatus() > 0 and attendance == nil then attendance = 1 end return attendance == 1 end,nil,nil))
prelPreflightProc:addItem(SimpleProcedureItem:new("Electrical Power Up supplementary procedure is complete."))
prelPreflightProc:addItem(ProcedureItem:new("IRS MODE SELECTORS","OFF",ChecklistItem.actorFO,1,function (self) if sysGeneral.irsUnit1Switch:getStatus() == 0 and irs1off == nil then irs1off = 1 end if sysGeneral.irsUnit2Switch:getStatus() == 0 and irs2off == nil then irs2off = 1 end return irs1off == 1 and irs2off == 1 end,nil,nil))
prelPreflightProc:addItem(ProcedureItem:new("IRS MODE SELECTORS","THEN NAV",ChecklistItem.actorFO,1,function (self) if sysGeneral.irsUnit1Switch:getStatus() == 2 and irs1nav == nil then irs1nav = 1 end if sysGeneral.irsUnit2Switch:getStatus() == 2 and irs2nav == nil then irs2nav = 1 end return irs1nav == 1 and irs2nav == 1 end,nil,nil))
...
-- ============ CDU Preflight =============
local cduPreflightProc = Procedure:new("CDU PREFLIGHT PROCEDURE (CAPT)")
